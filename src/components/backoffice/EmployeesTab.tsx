...
import { UserPlus, KeyRound, ToggleLeft, ToggleRight, X, Eye, EyeOff, LoaderCircle } from 'lucide-react';
            ...
            {employees.map(emp => (
              <tr key={emp.id} className="border-b border-slate-50 last:border-0 hover:bg-slate-50/50 transition">
                ...
                <td className="px-4 py-3">
                  <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${ROLE_COLORS[emp.role] ?? 'bg-slate-100 text-slate-500'}`}>
                    {emp.role}
                  </span>
                </td>
                <td className="px-4 py-3">
                  <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${emp.shift_status === 'active' ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500'}`}>
                    {emp.shift_status}
                  </span>
                </td>
                <td className="px-4 py-3">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {emp.id !== currentEmployee.id && (
                        <button
                          ...
                          className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-semibold transition ${emp.is_active ? 'bg-rose-50 text-rose-500 hover:bg-rose-100' : 'bg-emerald-50 text-emerald-600 hover:bg-emerald-100'}`}
